...
import { FlatList, View, ScrollView, StyleSheet, StatusBar } from 'react-native'
import {
...
class SearchPage extends Component {
    ...
    return (
      <View style={{ paddingBottom: hp('1%') }}>
        <Text
          style={[
            ApplicationStyles.fontStyles.caption,
            { textAlign: 'left', paddingHorizontal: wp('2%') },
          ]}
        >
          {title}
        </Text>
        <FlatList
          data={items}
          renderItem={
            type === 'ngo' ? this.renderItem : (data) => this.renderPostItem({ ...data, type })
          }
        />
        <Button
          title="See all"
          ...
          onPress={() => this.openSeeAllScreen(type)}
        />
      </View>
    )
  }
  ...
  renderPostItem({ item, type }) {
    return (
      <Button
        buttonWrapperStyle={[styles.subContainer]}
        onPress={() => this.openSeeAllScreen(type, item._id)}
      >
        <AvatarImage
        ...
        <View style={[styles.avatarContainer]}>
          <View style={{ flexDirection: 'row', flex: 1 }}>
            <Text style={ApplicationStyles.fontStyles.body1}>{item.title}</Text>
          </View>
          <View style={styles.agoContainer}>
            <Text style={styles.ago}>By {item.userId.name}</Text>
          </View>
        </View>
      </Button>
    )
      ...
      >
        ...
        <View style={[styles.avatarContainer]}>
          <View style={{ flexDirection: 'row', flex: 3 }}>
            <Text style={ApplicationStyles.fontStyles.body2}>{item.name}</Text>
          </View>
          <View style={styles.agoContainer}>
            <Text style={styles.ago}>
              {CommonFunctions.getFollowerCount(item.followerCount)}{' '}
              {CommonFunctions.getPluralString('Follower', item.followerCount)}
            </Text>
          </View>
        </View>
      </Button>
  ...
  render() {
    ...
    const searchResultExist =
      (autoComplete.posts && autoComplete.posts.length > 0) ||
    ...
    return (
      ...
      >
        <Dialog
          ...
          dialogTitle={
            <DialogTitle title="Welcome!" textStyle={{ ...ApplicationStyles.fontStyles.caption }} />
          }
        >
          <DialogContent style={{ width: wp('80%'), height: hp('25%') }}>
            <View style={{ flex: 1, flexDirection: 'column' }}>
              ...
              >
                Thank you so much {profile && profile.name}, for becoming the part of Handout
                family. We together make this world a better place for everyone.{' '}
              </Text>
              <Text
                style={{
                  ...ApplicationStyles.fontStyles.caption,
              ...
              >
                To get started, Please follow some philanthropy organistaions. We have given some
                suggestions below and you can also search yours.
              </Text>
            </View>
          </DialogContent>
        </Dialog>
        ...
        >
            ...
            {['POST', 'EVENT', 'PO', 'USER'].map((o) => (
              <Button
                style={{
                  ...
                  borderBottomColor:
                    selectedTab === o
                      ? ApplicationStyles.primaryColor.color
                      : ApplicationStyles.grayishBackground.color,
                }}
                ...
                <Text
                  style={{
                    ...
                    color:
                      selectedTab === o
                        ? ApplicationStyles.primaryColor.color
                        : ApplicationStyles.darkColor.color,
                  }}
                >
                  {o}
                </Text>
          ...
          <View style={{ flex: 1 }}>
            {['POSTS', 'EVENTS', 'SHOP'].includes(selectedTab) ? (
              <View
                style={{
                  flex: 1,
                  flexDirection: 'row',
              ...
              >
                <FlatList
                  ...
                  renderItem={({ item }) => (
                    <Button
                      onPress={() =>
                        this.openSeeAllScreen(selectedTab === 'POSTS' ? 'post' : 'event', item._id)
                      }
                      buttonWrapperStyle={{
                        width: wp('28%'),
                        height: wp('28%'),
              ...
              >
                <FlatList
                  ...
                  onMomentumScrollBegin={() => {
                    this.onEndReachedCalledDuringMomentumPo = false
                  }}
                  onEndReached={(data) => {
                  ...
                  renderItem={({ item }) => (
                    ...
                    >
                      ...
                      />
                      <Text style={{ textAlign: 'center', ...ApplicationStyles.fontStyles.body2 }}>
                        {item.name}
                      </Text>
                    </Button>
                  )}
          ...
          {searchResultExist && (
            ...
            >
              ...
              {autoComplete.ngos &&
                autoComplete.ngos.length > 0 &&
                this.searchSection('NGOs', 'ngo', autoComplete.ngos)}
              {autoComplete.posts &&
                autoComplete.posts.length > 0 &&
                this.searchSection('Posts', 'post', autoComplete.posts)}
              {autoComplete.events &&
                autoComplete.events.length > 0 &&
                this.searchSection('Events', 'event', autoComplete.events)}
            </View>
          )}
        </View>
      </View>
