...
class Drawer extends Component {
      ...
      >
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <Icon
            name={leftIcon.name}
            style={{ width: wp('6.8%') }}
            size={leftIcon.size || wp('6%')}
            color={leftIcon.color || ApplicationStyles.info3.color}
            iconFamily={leftIcon.family || 'Ionicons'}
          />
          <Text
            style={{
  ...
  render() {
    ...
    return (
      <ScrollView
        style={{
          ...ApplicationStyles.elevationXLL,
          flex: 1,
      ...
      >
        ...
        >
          ...
          <View style={{ marginLeft: wp('2%'), justifyContent: 'center' }}>
            ...
            <Text
              style={{ ...ApplicationStyles.fontStyles.caption, textAlign: 'left' }}
              onPress={() => {
                closeDrawer()
        ...
        {profile && profile.role === 'user' ? (
          <View style={{ marginTop: hp('3%') }}>
            {this.getMenuItem(
              'FOLLOWINGS',
              { name: 'user-following', family: 'SimpleLineIcons', size: wp('5%') },
              () => navigate('Followings')
            )}
            {/* { this.getMenuItem('Handouts', {name: 'hand-paper-o', family: 'FontAwesome', size: wp('5%')}) } */}
            {this.getMenuItem(
              'DONATIONS',
              { name: 'money', family: 'FontAwesome', size: wp('4.9%') },
              () => navigate('MyDonations')
            )}
            {this.getMenuItem(
              'RECURRING DONATIONS',
              { name: 'sync', family: 'AntDesign', size: wp('4.6%') },
              () => navigate('RecurringDonations')
            )}
            {this.getMenuItem(
              'SETTINGS',
              { name: 'setting', family: 'AntDesign', size: wp('5.4%') },
              () => navigate('Settings')
            )}
          </View>
        ) : (
          <View style={{ marginTop: hp('3%') }}>
            {this.getMenuItem('FOLLOWERS', {
              name: 'user-following',
            ...
            {this.getMenuItem(
              'DONATIONS',
              { name: 'money', family: 'FontAwesome', size: wp('4.9%') },
              () => navigate('MyDonations', { forPo: true })
            )}
            {this.getMenuItem(
              'RECURRING DONATIONS',
              { name: 'sync', family: 'AntDesign', size: wp('4.6%') },
            ...
            {this.getMenuItem(
              'PAYOUTS',
              { name: 'money', family: 'FontAwesome', size: wp('4.6%') },
            ...
            {this.getMenuItem(
              'SETTINGS',
              { name: 'setting', family: 'AntDesign', size: wp('5.3%') },
              () => navigate('Settings')
            )}
          </View>
        )}
...
export default connect(({ user: { profile } }) => ({ profile }), {
