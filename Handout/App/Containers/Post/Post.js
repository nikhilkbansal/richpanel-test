...
class Post extends Component {
  ...
  _renderItem = ({ item, index }) => (
    <PostUi
      ...
      onRepost={() =>
        this.props.navigation.navigate('AddPost', { itemId: item._id, isRepost: true })
      }
      onReactionPress={this.props.postReaction}
      onReactionRemovePress={this.props.removeReaction}
      userPicture={item.userId.picture}
      onViewComments={() =>
        this.props.navigation.navigate('Comment', { itemId: item._id, itemType: 'post' })
      }
      onUserClick={() =>
    ...
    return (
      <View style={{ flex: 1, backgroundColor: ApplicationStyles.smokeBackground.color }}>
        <NavigationBar
          ...
          leftIcon={'md-menu'}
          rightButtonAction={() => navigation.navigate('AddPost')}
